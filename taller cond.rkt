...
(define (valorservicios x)
  (cond((and(>= x 1)(<= x 2)) (* 80000 0.95))
       ((and(>= x 3)(<= x 4)) (* 80000 0.90))
       ((and(>= x 5)(<= x 6)) (* 80000 0.80))
       )
  )
...
(define (principal14)
  (begin
    (display "Digite su nombre: ")
    (define nombre (read))
    (display "Digite su direccion: ")
    (define direccion (read))
    (display "Digite su estrato de 1 a 6: ")
    (define x(read))
    (display "Su total a pagar de servicios publicos, seria: ")
    (valorservicios x)
    )
  )
